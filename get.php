	...
	if ($student) { // если студент есть в базе
		...
		if ($points < $passing_score) { // если студент не прошел тест
			...
			if ($bonus == 3 && !$test_complite) { // если тест EASY пройден в первый раз через 5мин отправляем картинки
				...
				switch ($quiz_title) { // проверка названия теста без окончания IN или OUT
					...
					default:
						$msg = "Название теста не подошло ни под одно из условий...\nвот что пришло - " . $quiz_title . "./nУченик с логином - " . $username . " не получит отбивку по истечении 5 минут";
